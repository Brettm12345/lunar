...
export class AstBuilder {
  ...
  private isUnary(operator?: operatorIds): operator is unaryOperator {
    ...
    return (
      this.isOperator(operator) && unary.includes(token.value as unaryOperator)
    )
  }
  ...
  private skipOperator(operator: operatorIds) {
    if (this.isOperator(operator)) {
      ...
      this.input.croak(`Expecting operator: "${operators[operator].value}"`)
    }
  }
        ...
        return this.maybeBinary(
          {
            ...
            left,
            right: this.maybeBinary(this.parseAtom(), nextOperatorImportance)
          },
          importance
        )
      }
  ...
  public parseAtom() {
    return this.maybeCall(() => {
      if (this.isPunctuation(punctuation.openParenthesis)) {
        ...
        [keywords.declare, keywords.const].map(this.isKeyword).includes(true)
      ) {
        return this.parseDefinitions()
      } else if (
        [keywords.true, keywords.false].map(this.isKeyword).includes(false)
      ) {
      ...
      if (
        token !== null &&
        [tokens.variable, tokens.number, tokens.string].includes(token.type)
      ) {
